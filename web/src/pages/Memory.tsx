    ...
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-theme-faint" />
          <input
            type="text"
            value={search}
        ...
        <div className="relative">
          <Filter className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-theme-faint" />
          <select
      ...
      {error && (
        <div className="rounded-xl p-3 text-sm text-[var(--color-status-error-soft)] animate-fade-in" style={{ background: 'color-mix(in srgb, var(--color-status-error) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--color-status-error) 22%, transparent)' }}>
          {error}
        </div>
      )}
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-theme-secondary mb-1.5 uppercase tracking-wider">
                  {t('memory.key_required')} <span className="text-[var(--color-status-error)]">*</span>
                </label>
                <input
                  type="text"
                  value={formKey}
              ...
              <div>
                <label className="block text-xs font-semibold text-theme-secondary mb-1.5 uppercase tracking-wider">
                  {t('memory.content_required')} <span className="text-[var(--color-status-error)]">*</span>
                </label>
                <textarea
                  value={formContent}
      ...
      {loading ? (
        ...
        <div className="glass-card p-8 text-center">
          <Brain className="h-10 w-10 text-[var(--color-border-strong)] mx-auto mb-3" />
          <p className="text-theme-muted">{t('memory.empty')}</p>
        </div>
      ) : (
