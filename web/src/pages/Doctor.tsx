    ...
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-theme-primary uppercase tracking-wider">{t('doctor.diagnostics_title')}</h2>
        </div>
        <button
          onClick={handleRun}
          disabled={loading}
      ...
      {/* Error */}
      {error && (
        <div className="rounded-xl p-4 text-[var(--color-status-error-soft)] animate-fade-in" style={{ background: 'color-mix(in srgb, var(--color-status-error) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--color-status-error) 22%, transparent)' }}>
          {error}
        </div>
      )}
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-16 animate-fade-in">
          <div className="theme-spinner h-12 w-12 rounded-full animate-spin mb-4" />
          <p className="text-theme-secondary">{t('doctor.running_desc')}</p>
          ...
          <div className="glass-card flex items-center gap-4 p-4 animate-slide-in-up">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-theme-primary font-medium">
                {okCount} <span className="text-theme-muted font-normal">ok</span>
              </span>
            </div>
                  ...
                  {items.map((result, idx) => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <p className="text-sm text-theme-primary">{result.message}</p>
                        <p className="text-[10px] text-theme-faint mt-0.5 capitalize uppercase tracking-wider">
                          {result.severity}
                        </p>
                      </div>
                    </div>
      ...
      {!results && !loading && !error && (
        <div className="flex flex-col items-center justify-center py-16 text-theme-faint animate-fade-in">
          ...
          <p className="text-lg font-semibold text-theme-primary mb-1">{t('doctor.system_diagnostics')}</p>
